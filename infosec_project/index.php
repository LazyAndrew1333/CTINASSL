...
header("Content-Security-Policy: default-src 'self'; script-src 'self'; style-src 'self' 'nonce-$nonce'; img-src 'self'; frame-ancestors 'none'; form-action 'self';");
...
// Redirect to custom error page on fatal errors
register_shutdown_function(function() {
    $error = error_get_last();
    if ($error && ($error['type'] === E_ERROR || $error['type'] === E_PARSE || $error['type'] === E_CORE_ERROR || $error['type'] === E_COMPILE_ERROR)) {
        echo "<script>alert('A fatal error occurred. Please try again later.');</script>";
        error_log("Fatal error: " . $error['message'] . " in " . $error['file'] . " on line " . $error['line'], 3, 'errors.log');
    }
});
